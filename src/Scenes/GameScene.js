...
class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.platform.body.setSize(
      ...
      this.platform.displayHeight - 10,
      true // center on sprite
    );
    ...
    this.physics.world.on(
      ...
      (body, up, down, left, right) => {
        if (body.gameObject === this.player && down) {
          // touching bottom only
          this._takeDamage(this.player);
        }
      },
      this
    );
  ...
  handlePlayerPlatform(player, platform) {
    ...
    const horizFactor = 1.2;
    let vx = targetSpeed * clamped * horizFactor;
